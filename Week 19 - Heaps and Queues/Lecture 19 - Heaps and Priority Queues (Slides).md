...
An example using the `PriorityQueue` class in Python.

**NOTE - This returns items based on the minimum!**
...
customers.put((2, "Harry", "123456")) # insert()
customers.put((3, "Charles")) # insert()
customers.put((1, "Riya")) # insert()
customers.put((2, "Stacy")) # insert()
customers.put((2, "Abid")) # insert()
while not customers.empty(): # corrected error on the webpage above
     print(customers.get()) # extract_min()

# prints in the following order - (1, 'Riya'), (2, 'Abid'), (2, 'Harry', '123456'), (2, 'Stacy'),  (3, 'Charles')
```

</div>

So, `(2, 'Abid') < (2, 'Harry', '123456') < (2, 'Stacy')` because Python first sorts on the `2`s and then the names `'Harry'`, `'Stacy'` and `'Abid'`.

---

# My Implementation

**NOTE - This returns items based on the maximum!**

- This uses the classic version from ***CLRS - Introduction to algorithms*** (see references).
...
```python
from priority_queue import PriorityQueueADT 

customers = PriorityQueueADT()

customers.insert((2, "Harry", "123456")) # insert()
customers.insert((3, "Charles")) # insert()
customers.insert((1, "Riya")) # insert()
customers.insert((2, "Stacy")) # insert()
customers.insert((2, "Abid")) # insert()

while not customers.is_empty(): # corrected error on the webpage above
  item = customers.extract_max() # extract_max()
  print(f"Key: {item.key}, Value: {item.value}")

# prints in the following order - (3, 'Charles'), (2, 'Stacy'), (2, 'Harry', '123456'), (2, 'Abid'), (1, 'Riya')
```
...
<div class="parent_child_img">



![](assets/parent_left_right_animation.png)

</div>

***These are highlighted on the next 3 slides...***
...
![](assets/parent_left_right_animation2.png)
...
![](assets/parent_left_right_animation3.png)
...
![](assets/parent_left_right_animation4.png)
...
- **Heap Property** (either Max-Heap Property or  Min-Heap Property).
- e.g. **Max-heap Property**.
  - For every node with index $i$.
    - `A[parent(i)] >= A[i]`
  That is, the value of each node is less than or equal than it's parents. 

<style>
#max_heap_property img{
  height:300px;
}
</style>

<div id="max_heap_property">
<center>

![](assets/max-heap-property.png)
